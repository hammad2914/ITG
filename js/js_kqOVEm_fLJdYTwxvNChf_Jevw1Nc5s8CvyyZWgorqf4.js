                ...
                * Hidden menu for development
                ...
                window.setTimeout(function () {
                  ...
                  if(typeof consent_item !== 'undefined') {
                    ...
                    if(lang === 'de') {
                      consent_item.innerText = 'Cookie-Präferenzen';
                    } else if(lang === 'fr') {
                      consent_item.innerText = 'Préférences de cookies';
                    } else {
...
jQuery(function ($) {
  ...
  function initSelect2Elements(loadDefault) {
      ...
      if($('.view-customers-index-view').find('[name=industry]').length > 0) {
          $('.view-customers-index-view').find('[name=industry]').select2(select2Options);
      }
      if($('.view-customers-index-view').find('[name=company_size]').length > 0) {
          $('.view-customers-index-view').find('[name=company_size]').select2(select2Options);
      }
  }
  ...
  jQuery(document).ajaxComplete(function(event,xhr,options) {
      ...
      if (typeof(options.extraData) != 'undefined') {
          ...
          if (options.extraData.view_path === '/customers') {
              initSelect2Elements(false);
          }
...
jQuery(document).ready(function() {
    jQuery('.tabs li').on('click', function() {
        var tab = jQuery(this).data('tab');
				jQuery(this).siblings('li.is-active').removeClass("is-active");
        jQuery(this).closest("li").addClass('is-active');
				jQuery(this).parent().parent().siblings().children().removeClass("is-active");
				jQuery(this).parent().parent().siblings().children('div[data-content="' + tab + '"]').addClass("is-active");
    });
});;
...
;
/**
 * Provides functionality for the slider-wheel component.
 *
 * @todo Better error handling for malformed SVGs.
 * @todo Add detach().
 */
(function (once, $) {
  ...
  // Get the SVG content document given a parent element containing just one.
  const getSvgDoc = e => e.querySelector('[data-slider-wheel-image]').contentDocument;

  /**
   * Ensures a callback is run on an <object> when it's loaded.
   *
   * FOR SOME REASON THIS DOESN'T ALWAYS WORK AS EXPECTED ON CHROME. Sometimes
   * the contentDocument shows as loaded when it turns out to be an empty HTML
   * document. No idea why, so for now do not use.
   *
   * @param {HTMLObjectElement} objectElement
   * @param {Function} callback
   */
  function ensureObjectCallback(objectElement, callback) {
  ...
  /**
   * Ensures a callback is run when the document and its resources are loaded.
   *
   * @param {Function} callback
   */
  ...
  /**
   * Attach the behavior to the context.
   *
  ...
  function attach(context) {
    ...
    /**
     * Set the appropriate active element to be visible.
     *
     * @param {HTMLElement} component - the slider wheel component.
     * @param {Number} slide - the slide index.
     */
    const setActiveIcon = (component, slide) => {
      // Update the active class within the SVG. It might not be loaded, eg.
      // on mobile.
      const svgDoc = getSvgDoc(component);
      if (svgDoc) {
        const dataSelector = `[data-slider-wheel-slide="${slide}"]`;
        svgDoc.querySelectorAll(`.activation.active:not(${dataSelector})`)
          .forEach(item => {
            item.classList.remove('active');
          });
        svgDoc.querySelectorAll(`.activation${dataSelector}:not(.active)`)
          .forEach(item => {
    ...
    once('slider-wheel', '[data-slider-wheel]', context).forEach(component => {
      const $slickElement = $('[data-slider-wheel-slides]', component);
      const $status = $('[data-slider-wheel-count]', component);

      // Add the init/change handler.
      $slickElement.on('init reInit beforeChange', function(event, slick, currentSlide, nextSlide){
        const slide = nextSlide || 0;
        // Update the slide counter.
        $status.text(`${slide + 1} / ${slick.slideCount}`);
        setActiveIcon(component, slide);
      });
      ...
      $slickElement.on('init', function (event, slick, currentSlide, nextSlide) {
        ...
        once('slider-wheel-click', '[data-slider-wheel-image]', component).forEach(image => {
          ensureCallback( e => {
      ...
      $slickElement.slick({
        arrows: false,
        dots: true,
        appendDots: component.querySelector('[data-slider-wheel-dots]'),
        responsive: [
          {
            // It looks like Slick needs the pixel value without unit for
            // responsiveness.
            breakpoint: 1100,
  ...
  /**
   * Behaviors for the slider-wheel component.
   *
   * @type {Drupal~behavior}
   *
   * @prop {Drupal~behaviorAttach} attach
   *   Attaches the slider-wheel behavior.
   */
  Drupal.behaviors.sliderWheel = {
    attach
  };
